...
contract MisbehaviorProver is Prover {
        ...
        returns (Proof memory, address, address, address, address, uint256, uint256)
    {
        ...
        uint256 newBalance = IERC20(token).balanceOf(relayer);
        ...
        newBalance -= IERC20(token).balanceOf(relayer); // Subtract old balance
        return (proof(), relayer, token, tokenDest, _owner, depositBlockId, newBalance);
    }
}
